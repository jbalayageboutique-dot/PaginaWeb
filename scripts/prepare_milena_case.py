...
CASE_INFO = {
    ...
    "title": "Mechas con papel y Babylights Rubio Cálido | Caso Milena",
    "description": "Trabajo técnico de mechas con papel combinadas con babylights muy finas para iluminar el contorno del rostro, logrando una altura de decoloración 9 con matices y tonos rubios cálidos espectaculares para Milena en Maison Balayage Studio, Punta Arenas.",
    "keywords": "mechas con papel, babylights Rubio Calido, iluminacion de contorno, rubio calido altura 9, mechas finas, peluqueria Punta Arenas, Maison Balayage Studio, colorista Punta Arenas",
}
...
def get_exif_gps(lat, lon):
    ...
    # Convert floats to (num, den) for piexif
    lat_deg_rational = (lat_deg, 1)
    lat_min_rational = (lat_min, 1)
    lat_sec_rational = (int(lat_sec * 10000), 10000)
    
    lon_deg_rational = (lon_deg, 1)
    lon_min_rational = (lon_min, 1)
    lon_sec_rational = (int(lon_sec * 10000), 10000)
    
    gps_ifd = {
        piexif.GPSIFD.GPSLatitudeRef: lat_ref.encode('utf-8'),
        piexif.GPSIFD.GPSLatitude: (lat_deg_rational, lat_min_rational, lat_sec_rational),
        piexif.GPSIFD.GPSLongitudeRef: lon_ref.encode('utf-8'),
        piexif.GPSIFD.GPSLongitude: (lon_deg_rational, lon_min_rational, lon_sec_rational),
    }
    return gps_ifd

def set_jpg_metadata(src, dst):
    # Set exif tags using piexif
    ...
    # Load original image and insert EXIF
...
def main():
    src_dir = Path("/Users/janet/Downloads/milena mechas babyligth")
    if not src_dir.exists():
        print(f"Error: Source directory {src_dir} does not exist.")
        return

    # Workspace paths
    ws_dir = Path("/Users/janet/Documents/IA/WEBSITE JBALAYAGE/milena-mechas-babylights-rubio-calido-punta-arenas")
    ...
    dl_dest_dir = Path("/Users/janet/Downloads/milena-mechas-babylights-rubio-calido-punta-arenas")
    ...
    mappings = [
        # Photo BEFORE
        {"src": "IMG_2179.jpg", "type": "photo", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-antes-del-cambio-01.jpg"},
        # Video BEFORE
        {"src": "IMG_2184.MOV", "type": "video", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-antes-del-cambio-01.mov"},
        # Video AFTER
        {"src": "693c55d14b3945e6910eb2ee9867bc35.MOV", "type": "video", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-movimiento-brillo-despues-01.mov"},
        # Photo PORTADA AFTER
        {"src": "083AFE93-6096-40F0-A861-4B70DF31E692.PNG", "type": "photo", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-rubio-calido-portada-despues-01.png"},
        # Photo GALLERY ESPEJO
        {"src": "6263DCB3-6069-4D4B-9FEC-0CFE0FD171DA.PNG", "type": "photo", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-rubio-calido-espejo-01.png"},
        # Photo GALLERY DETALLE
        {"src": "070839C4-FD1E-4717-A213-5679DE3DB3E3.PNG", "type": "photo", "new_name": "maison-balayage-punta-arenas-milena-mechas-babylights-rubio-calido-detalle-01.png"},
    ]

    manifest = []
    
    print("Iniciando procesamiento de archivos para el caso Milena...")
    ...
    manifest_path = ws_dir / "metadata-manifest-milena-mechas.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
    
    # Write README.md to workspace
    readme_content = f"""# Caso Milena — Mechas con papel y Babylights Rubio Cálido

## Resumen del Caso

- **Nombre del Cliente:** Milena
- **Historial:** Milena ha sido clienta de Maison Balayage Studio durante aproximadamente 3 años, haciéndose siempre su técnica signature de mechas.
- **Problemática/Objetivo:** Milena es rubia y buscaba realizar una transición hacia un rubio más cálido y luminoso, manteniendo el contraste y movimiento.
- **Procedimiento técnico:** 
  - Mechas con papel de plata para aclarar y dar dimensión.
  - Combinado con la técnica de **Babylights** muy finitas enfocadas en el contorno del rostro (contouring) para iluminar y suavizar los rasgos faciales.
  - Altura de decoloración alcanzada: **Nivel 9**.
  - Matizado con colores cálidos seleccionados para un acabado luminoso, saludable y espectacular.
