...
const caseDir = path.join(root, 'milena-mechas-babylights-rubio-calido-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'milenaCloudinary.json');
...
const folder = 'maison-balayage/casos/milena-mechas-babylights-rubio-calido-punta-arenas';
const imageExtensions = new Set(['.jpg', '.jpeg', '.png', '.webp', '.heic']);
const videoExtensions = new Set(['.mov', '.mp4', '.m4v']);

const caseContext = {
  title: 'Mechas con papel y Babylights Rubio Calido | Caso Milena',
  caption: 'Trabajo tecnico de mechas con papel y babylights muy finas para iluminar el contorno del rostro, logrando una altura 9 rubio calido.',
...
const payload = {
  ...
  case: {
    ...
    title: 'Mechas con papel y Babylights Rubio Calido Milena',
