...
const caseDir = path.join(root, 'roxana-balayage-cobrizo-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'roxanaCloudinary.json');
...
const folder = 'maison-balayage/casos/roxana-balayage-cobrizo-punta-arenas';
...
const caseContext = {
  ...
  caption: 'Trabajo de diseño de balayage cobrizo con reflejos caramelo en base oscura, logrando luminosidad y tridimensionalidad.',
...
const payload = {
  ...
  case: {
    ...
    title: 'Balayage Cobrizo y Caramelo Roxana',
