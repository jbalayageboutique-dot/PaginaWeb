...
const caseDir = path.join(root, 'carolina-balayage-rubio-calido-olaplex-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'carolinaNewCloudinary.json');
...
const folder = 'maison-balayage/casos/carolina-balayage-rubio-calido-olaplex-punta-arenas';
const imageExtensions = new Set(['.jpg', '.jpeg', '.png', '.webp', '.heic']);
const videoExtensions = new Set(['.mov', '.mp4', '.m4v']);

const caseContext = {
  title: 'Balayage Rubio Calido con Olaplex | Caso Carolina Rubio',
  caption: 'Transformacion de cabello oscuro natural a un balayage rubio calido luminoso con contorno iluminado. Tratamiento de Olaplex, sellado de cuticula e hidratacion profunda.',
...
function roleFor(file) {
  ...
  if (name.includes('espejo-ia')) return 'mirror-ia';
  if (name.includes('detalle-ia')) return 'detail-ia';
...
const payload = {
  ...
  case: {
    ...
    title: 'Balayage Rubio Calido con Olaplex Carolina Rubio',
