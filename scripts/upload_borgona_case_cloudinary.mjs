...
const caseDir = path.join(root, 'gisela-barrido-color-borgona-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'borgonaCloudinary.json');
...
const folder = 'maison-balayage/casos/gisela-barrido-color-borgona-punta-arenas';
...
const caseContext = {
  ...
  caption: 'Trabajo de barrido de color para retirar tonos anteriores y aplicacion de un color borgoña intenso y luminoso en un cabello voluptuoso y largo.',
...
const payload = {
  ...
  case: {
    ...
    title: 'Barrido de color y Borgoña Gisela',
