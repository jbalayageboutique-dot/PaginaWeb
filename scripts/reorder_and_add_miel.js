...
const filePath = path.resolve('src/data/balayageData.ts');

const carolinaRubioCalido = `  {
...
const nicolePeruMorenaIluminada = `  {
...
const reorderedCases = [
  ...
  roxanaRubio,
  elenaTurquesa,
  carolinaRubioWarmHoney,
  giselaBorgona,
  gabrielaCrespoLargo,
  nicolePeruMorenaIluminada,
  valentinaRubioLuminoso,
  mechasValentinaRubio,
  guindaCarameloMiel,
  ninoskaMiel,
  fernandaCase,
  milenaRubio,
  ...
  canasCamouflageCase,
  dinkaCase,
  mielDoradoCase, // Miel Dorado Case cleanly inserted here!
  case4,
  case1,
  case2,
  case3,
...
async function run() {
  ...
  // Find BEFORE_AFTER_CASES definition start and end
  const startKeyword = 'export const BEFORE_AFTER_CASES: BeforeAfterCase[] = [';
  const startIndex = content.indexOf(startKeyword);
  if (startIndex === -1) {
    throw new Error('Could not find start of BEFORE_AFTER_CASES');
  }

  const endKeyword = 'export const SERVICES_MENU: ServiceItem[] = [';
  const endIndex = content.indexOf(endKeyword);
  if (endIndex === -1) {
    throw new Error('Could not find start of SERVICES_MENU');
  }

  // Find the closing bracket before SERVICES_MENU
  const arrayContentEnd = content.lastIndexOf('];', endIndex);
  if (arrayContentEnd === -1 || arrayContentEnd < startIndex) {
    throw new Error('Could not find array end bracket');
  }

  const newArraySection = `${startKeyword}\n${reorderedCases}\n];\n\n`;
  const beforeSection = content.substring(0, startIndex);
  const afterSection = content.substring(arrayContentEnd + 2); // after "];"

  const updatedContent = beforeSection + newArraySection + afterSection;
  await fs.writeFile(filePath, updatedContent, 'utf8');
  console.log('Successfully added Miel Dorado case study cleanly to balayageData.ts!');
}
