...
const caseDir = path.join(root, 'carolina-cobrizo-correccion-color-cobrizo-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'carolinaCobrizoCloudinary.json');
...
const folder = 'maison-balayage/casos/carolina-cobrizo-correccion-color-cobrizo-punta-arenas';
const imageExtensions = new Set(['.jpg', '.jpeg', '.png', '.webp', '.heic']);
const videoExtensions = new Set(['.mov', '.mp4', '.m4v']);

const caseContext = {
  title: 'Correccion de color cobrizo en Punta Arenas | Caso Carolina Cobrizo',
  caption: 'Barrido de color sobre tintura negra y roja previa para lograr un cobrizo uniforme y luminoso.',
...
const payload = {
  ...
  case: {
    id: 'carolina-cobrizo-correccion-color-cobrizo-punta-arenas',
    title: 'Correccion de color cobrizo Carolina Cobrizo',
