...
const caseDir = path.join(root, 'nicole-morena-iluminada-punta-arenas');
const photosDir = path.join(caseDir, 'fotos');
const videosDir = path.join(caseDir, 'videos');
const outputPath = path.join(root, 'src', 'data', 'nicoleCloudinary.json');
...
const folder = 'maison-balayage/casos/nicole-morena-iluminada-punta-arenas';
...
const caseContext = {
  ...
  caption: 'Trabajo de transicion a Morena Iluminada en base oscura con acumulacion de tintura negra y roja. Prueba de mechas previa para evaluar la salud de la fibra capilar.',
